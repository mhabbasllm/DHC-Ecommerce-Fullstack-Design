...
const Cart = ({ onNavigate }) => {
  ...
  return (
    <main className="bg-[#f7f8fa] py-6 text-brand-dark font-sans min-h-[70vh]">
      <div className="container max-w-[1200px] mx-auto px-4 md:px-4 px-1">
        
        {/* ====== MOBILE CART HEADER ====== */}
        <div className="md:hidden flex items-center py-3 px-2 mb-2">
          <button onClick={() => onNavigate('home')} className="text-brand-dark p-1 mr-3 cursor-pointer">
            <ArrowLeft size={22} />
          </button>
          <h2 className="text-lg font-semibold text-brand-dark">Shopping cart</h2>
        </div>
        ...
        <div className="grid grid-cols-12 gap-6 items-start">
              ...
              {cartItems.length === 0 ? (
                ...
                <div className="flex flex-col">
                  {cartItems.map((item) => (
                    <div 
                      key={item.id} 
                      className="flex flex-col sm:flex-row gap-5 py-5 border-b border-gray-200 last:border-0 items-start justify-between relative"
                    >
                      {/* Mobile 3-dots menu */}
                      <button className="md:hidden absolute right-0 top-5 text-[#9ca3af] p-1 cursor-pointer hover:bg-gray-100 rounded-full">
                      ...
                      <div className="flex gap-4 flex-1">
                        {/* Thumbnail image */}
                        <div className="w-[80px] h-[80px] border border-gray-200 rounded-md flex items-center justify-center p-1.5 flex-shrink-0 bg-white select-none overflow-hidden">
                          <img 
                            src={item.img} 
                            alt={item.title} 
                            className={`max-h-full max-w-full object-contain ${
                              item.img === cloth6 ? 'scale-[1.15]' : 
                        ...
                        {/* Text descriptions */}
                        <div className="flex flex-col pr-6 md:pr-0">
                          <h4 className="text-sm md:text-base font-semibold text-brand-dark hover:text-brand-blue cursor-pointer transition-colors leading-snug" onClick={() => onNavigate('product-details')}>
                            {item.title}
                          </h4>
                          <span className="text-[12px] text-brand-gray mt-1 leading-tight select-none">
                            Size: {item.size}, Color: {item.color}, Material: {item.material}
                          </span>
                          <span className="text-[12px] text-brand-gray mt-0.5 select-none">
                            Seller: {item.seller}
                          </span>
                      ...
                      <div className="flex sm:flex-col items-center sm:items-end gap-3 sm:gap-2.5 w-full sm:w-auto mt-4 sm:mt-0 pt-3 sm:pt-0 border-t sm:border-t-0 border-gray-100 justify-between select-none">
                        
                        {/* Quantity Controls (order 1 on mobile, 2 on desktop) */}
                        <div className="order-1 md:order-2">
                          {/* Quantity increment/decrement buttons (Mobile only) */}
                          <div className="flex md:hidden items-center border border-gray-300 rounded-md overflow-hidden bg-white shadow-sm h-[32px]">
                            <button 
                              className="px-3 h-full flex items-center justify-center text-gray-500 hover:bg-gray-100 transition-colors cursor-pointer disabled:opacity-50"
                              onClick={() => handleQtyChange(item.id, Math.max(1, item.qty - 1))}
                              disabled={item.qty <= 1}
                            >
                              <Minus size={14} />
                            </button>
                            <div className="w-10 h-full flex items-center justify-center border-x border-gray-300 text-sm font-medium text-brand-dark">
                              {item.qty}
                            </div>
                            <button 
                              className="px-3 h-full flex items-center justify-center text-gray-500 hover:bg-gray-100 transition-colors cursor-pointer"
                        ...
                        <span className="text-base md:text-lg font-semibold text-brand-dark block order-2 md:order-1">
          ...
          <div className="col-span-12 lg:col-span-3 flex flex-col gap-4 select-none">
            ...
            <div className="border-none bg-transparent md:border-solid md:border md:border-gray-200 rounded-md p-0 md:p-5 md:bg-white shadow-none md:shadow-sm flex flex-col gap-3.5 text-sm">
              ...
              {/* Mobile labels */}
              <div className="md:hidden flex justify-between items-center text-brand-gray">
                <span>Items ({cartItems.reduce((acc, i) => acc + i.qty, 0)}):</span>
                <span className="font-medium text-brand-dark">${subtotal.toFixed(2)}</span>
              </div>
              <div className="md:hidden flex justify-between items-center text-brand-gray">
                <span>Shipping:</span>
                <span className="font-medium text-brand-dark">$10.00</span>
              </div>

              {/* Shared Tax */}
              <div className="flex justify-between items-center text-brand-gray">
                <span>Tax:</span>
                <span className="font-medium text-brand-dark md:text-[#00b517] font-semibold md:font-semibold">${tax.toFixed(2)}</span>
              </div>
              ...
              <div className="flex justify-between items-baseline pt-1">
                ...
                <span className="font-bold text-brand-dark text-xl leading-none">
                  ${total.toFixed(2)}
                </span>
              </div>
              ...
              <button 
                ...
                className="bg-[#00b517] hover:bg-[#009b13] text-white rounded-md py-3 text-center font-bold text-base cursor-pointer shadow-sm transition-colors w-full mt-4"
              >
        ...
        <div className="mt-10 select-none">
          <h3 className="font-bold text-lg md:text-xl text-brand-dark mb-4">
            Saved for later
          </h3>
          
          {savedItems.length === 0 ? (
            <div className="text-center py-6 text-brand-gray text-sm bg-white border border-gray-200 rounded-md select-none">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-5">
              {savedItems.map((item) => (
                <div 
                  key={item.id} 
                  className="bg-white border border-gray-200 rounded-md p-4 shadow-sm hover:shadow-md transition-all group flex flex-row md:flex-col items-center md:items-stretch gap-4 md:gap-0 cursor-pointer"
                >
                  {/* Item Image */}
                  <div className="w-[80px] h-[80px] md:w-full md:h-[140px] flex items-center justify-center md:p-3 select-none md:mb-3 overflow-hidden bg-white flex-shrink-0 rounded-md">
                    <img 
                      src={item.img} 
                      alt={item.title} 
                      className={`max-h-[90%] max-w-[90%] object-contain transition-transform duration-300 group-hover:scale-105 ${
                        item.img === tech7 ? 'scale-[1.35]' : 
                  ...
                  <div className="flex-1 flex flex-col justify-between h-full">
                    <div className="flex flex-col">
                      <span className="text-base font-bold text-brand-dark block order-2 md:order-1 mt-1 md:mt-0">
                        ${item.price.toFixed(2)}
                      </span>
                      <h4 className="text-sm md:text-xs text-brand-gray md:group-hover:text-brand-blue transition-colors leading-snug line-clamp-2 order-1 md:order-2 md:mt-1">
                        {item.title}
                      </h4>
                    </div>
                    ...
                    <div className="flex items-center gap-2 mt-3 md:mt-4">
                      ...
                      <button 
                        ...
                        className="flex-1 border border-brand-blue md:border-gray-300 md:hover:border-brand-blue text-brand-blue rounded-md py-1.5 md:py-2 text-center text-xs font-semibold transition-all bg-white flex items-center justify-center gap-2 cursor-pointer hover:bg-blue-50"
                      >
                      ...
                      <button 
                        ...
                        className="md:hidden flex-1 border border-gray-300 text-[#eb001b] rounded-md py-1.5 text-center text-xs font-medium transition-all bg-white cursor-pointer hover:bg-red-50"
                      >
                        Remove
                      </button>
                    </div>
        ...
        <div className="bg-gradient-to-r from-brand-blue to-blue-700 rounded-md p-7 text-white flex flex-col md:flex-row justify-between items-center gap-6 shadow-md relative overflow-hidden select-none mt-10">
          ...
          <div>
            ...
            <p className="text-white opacity-85 text-sm mt-1.5">
              Have you ever finally just write dummy info
            </p>
          </div>
          
          <button className="bg-[#ff9017] hover:bg-orange-600 text-white font-bold py-2.5 px-6 rounded-md transition-colors shadow-md flex-shrink-0 cursor-pointer">
            Shop now
          </button>
        </div>
